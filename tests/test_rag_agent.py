...
def test_create_rag_agent_invokes_chain(
    ...
    O teste substitui a factory ``_make_embeddings`` por uma que devolve
    ``FakeEmbeddings``, constrói o índice FAISS sobre os docs reais de
    ``data/docs/`` e verifica que a resposta final passou pelo
    ``format_response``.
    """
    import agents.rag_agent as rag_mod

    monkeypatch.setattr(rag_mod, "_make_embeddings", lambda model=None: fake_embeddings)
